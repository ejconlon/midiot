...
-- import Midiot.Mem (sbsToVec, vecToSbs)
-- import Midiot.Msg (ChanVoiceMsg (..), ChanVoiceMsgData (..), Channel (..), ControlNum (..), ControlVal (..),
--                    MidiEvent (..), MidiMsg (..), MidiParsed (..), Note (..), PitchBend (..), Pressure (..),
--                    ProgramNum (..), Velocity (..))
-- import Midiot.Time (timeDeltaFromFracSecs)

-- data ShortBytes =
--     ShortBytes0
--   | ShortBytes1 !Word8
--   | ShortBytes2 !Word8 !Word8
...
-- decodeChanVoice :: Word8 -> ShortBytes -> Maybe ChanVoiceMsgData
...
-- decodeOther :: Word8 -> ShortBytes -> Maybe MidiParsed
...
-- decodeMsg bytes = maybe (UnparsedMidiMsg (vecToSbs bytes)) ParsedMidiMsg res where
--   res
...
-- encodeMsg msg =
--   case msg of
--     UnparsedMidiMsg bytes -> sbsToVec bytes
